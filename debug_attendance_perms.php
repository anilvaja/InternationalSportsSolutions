...
echo "\n=== What AttendanceResource expects vs what exists ===\n";
$expectedPermissions = [
    'view_attendance',
    'create_attendance', 
    'edit_attendance',
...
foreach ($expectedPermissions as $expectedPerm) {
    $sql = "SELECT id FROM academy_permissions WHERE name = ?";
    $stmt = $db->prepare($sql);
    $stmt->execute([$expectedPerm]);
    $permId = $stmt->fetchColumn();
    
    if ($permId) {
        echo "✅ '{$expectedPerm}' exists in database (ID: {$permId})\n";
    } else {
        echo "❌ '{$expectedPerm}' NOT FOUND in database\n";
        
        // Check if plural version exists
        $pluralPerm = str_replace('attendance', 'attendances', $expectedPerm);
        $sql = "SELECT id FROM academy_permissions WHERE name = ?";
        $stmt = $db->prepare($sql);
        $stmt->execute([$pluralPerm]);
        $pluralPermId = $stmt->fetchColumn();
        
        if ($pluralPermId) {
            echo "   But '{$pluralPerm}' exists (ID: {$pluralPermId})\n";
        }
    }
}
